...
/**
 * This is a template GUI that you can use for your mapping program. It is an *abstract class*,
 * which means you'll need to extend it in your own program. For a simple example of how to do
 * this, have a look at the SquaresExample class.
 * 
 * This GUI uses Swing, not the first-year UI library. Swing is not the focus of this course,
 * but it would be to your benefit if you took some time to understand how this class works.
 * 
 * @author tony
 */
public abstract class GUI {
    /**
     * defines the different types of movement the user can perform, the appropriate one is
     * passed to your code when the move(Move) method is called.
     */
    public enum Move {
    ...
    /**
     * Is called when the drawing area is redrawn and performs all the logic for the actual
     * drawing, which is done with the passed Graphics object.
     */
    ...
    /**
     * Is called whenever the search box is updated. Use getSearchBox to get the JTextField
     * object that is the search box itself.
     */
    ...
    /**
     * Is called when the user has successfully selected a directory to load the data files
     * from. File objects representing the four files of interested are passed to the method.
     * The fourth File, polygons, might be null if it isn't present in the directory.
     * 
     * @param nodes
    ...
    /**
     * Description: <br/>
     * The A star search algorthim.
     * 
     * @author Yun Zhou
     */
    protected abstract void onAStarSearch();

    /**
     * Description: <br/>
     * Articulation points algorthim.
     * 
     * @author Yun Zhou
     */
    ...
    /**
     * @return the JTextField used as a search box in the top-right, which can be queried for
    ...
    /**
     * Redraws the window (including drawing pane). This is already done whenever a button is
     * pressed or the search box is updated, so you probably won't need to call this.
     */
    public void redraw() {
        frame.repaint();
    }
     ...
     * the component inheritance hierarchy. A JFrame is a component that represents the outer
     * window frame (with the minimise, maximise, and close buttons) of your program. Every
     * swing program has to have one somewhere. JFrames can, of course, have other components
     * inside them. JPanels are your bog-standard container component (can have other
     * components inside them), that are used for laying out your UI.
     */
    ...
    private void initialise() {
        ...
        navigation.setLayout(new GridLayout(2, 4));// change it from 3 to 4, so each row can
                                                   // hold 4 buttons
        navigation.add(out);
        ...
        /*
         * then make the JTextArea that goes down the bottom. we put this in a JScrollPane to
         * get scroll bars when necessary.
         */
        ...
        /*
         * finally, make the outer JFrame and put it all together. this is more complicated
         * than it could be, as we put the drawing and text output components inside a
         * JSplitPane so they can be resized by the user. the JScrollPane and the top bar are
